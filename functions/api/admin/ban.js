...
export async function onRequestPost({ request, env }) {
  try {
    ...
    await env.DB.prepare('INSERT OR IGNORE INTO banned_users (username, banned_at) VALUES (?, ?)').bind(username, Date.now()).run();
    // Moderator announcement
    await env.DB.prepare('INSERT INTO messages (username, message, created_at, type) VALUES (?, ?, ?, ?)').bind('system', username + ' has been banned.', Date.now(), 'system').run();
