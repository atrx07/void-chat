...
var exp = now + 43200;
var header = btoa(JSON.stringify({ typ: ‘JWT’, alg: ‘ES256’ })).replace(/+/g,’-’).replace(///g,’*’).replace(/=+$/,’’);
var payload = btoa(JSON.stringify({ aud: audience, exp: exp, sub: ‘mailto:admin@void.chat’ })).replace(/+/g,’-’).replace(///g,’*’).replace(/=+$/,’’);
var unsigned = header + ‘.’ + payload;
var keyData = Uint8Array.from(atob(privateKey.replace(/-/g,’+’).replace(/*/g,’/’)), function(c) { return c.charCodeAt(0); });
...
var sigB64 = btoa(String.fromCharCode.apply(null, new Uint8Array(sig))).replace(/+/g,’-’).replace(///g,’*’).replace(/=+$/,’’);
